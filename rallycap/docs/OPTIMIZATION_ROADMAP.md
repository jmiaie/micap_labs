...
| 2 | **WP model calibration on historical play-by-play** | The four hand-set constants (`SIGMA_PER_HALF_INNING`, run rates, tie-break, RE24 vintage) are roughly right, not fitted. Calibration is the M1 gate: model Brier must match/beat the market's on a holdout. | **Tooling done** (`rallycap calibrate` fits all four from line scores; `brier_score` for the gate) — needs Retrosheet/Statcast data (network) to run for real |
...
| # | Optimization | Why it matters | Status |
|---|---|---|---|
| 6 | **Resting-order fills in paper/backtest** | The original "passive never fills" assumption understates the passive path; modeling fills lets passive entries (which pay no spread) carry their weight in validation. | **v0 done** — passive buys rest with cash reserved, fill only on strict trade-through (worst-queue assumption), cancel-on-event wired in bot and engine, pending orders count against risk caps. Queue modeling from real book deltas remains M2 |
